...
public class ToolCardController {
	...
	public ToolCardController(PayStoneController psc, DbToolCardCollector tcc, DiceHolderController dhc,
			GameController gc) {
		this.dhc = dhc;
		this.tcc = tcc;
		this.psc = psc;
						...
						if (selectedToolcard == 0) {
							...
							if (cardpane.getCardNr() == 2) { // works reliable
								...
								if (dhc.getAmountOfDice() > 0) {
									...
									dhc.setTypeToInteractable(DiceHolderType.PLAYERWINDOW, true); // make it so you can
																									// only move in the
																									// window
							...
							if (cardpane.getCardNr() == 4) { // fix the test to see if window has at least two dices
																...
																// make it so it doesnt count its previous location as a
																// dice
																// make it so you cant use your normal turn to move in
																// the window (if you use it after activation)
							...
							if (cardpane.getCardNr() == 5) {
								if (dhc.GetSelectedDiceHolder() != null) {
									if (dhc.GetSelectedDiceHolder().getDie() != null) { // check if it has a die
										if (dhc.GetSelectedDiceHolder().getType() == DiceHolderType.OFFER) { // if it
																												// has a
																												// die,
																												// then
																												// check
										...
										if (dhc.GetSelectedDiceHolder().getType() == DiceHolderType.OFFER) {
											{
												...
												psc.pay(cardpane.getCardNr(),
														tcc.getPrice(cardpane.getCardNr(), gameid));
											}
										}
									}
								}
									...
									if (dhc.GetSelectedDiceHolder().getDie() != null) { // check if it has a die
										if (dhc.GetSelectedDiceHolder().getType() == DiceHolderType.OFFER) { // if it
											...
											if (dhc.getAmountOfDice() > 0) {
												...
												exception = true;
												dhc.setAllUninteractable();
												dhc.setTypeToInteractable(DiceHolderType.PLAYERWINDOW, true); // make it
																												// so
																												// you
																												// can
																												// only
								...
								if (dhc.GetSelectedDiceHolder() != null) {
									if (dhc.GetSelectedDiceHolder().getDie() != null) { // check if it has a die
										if (dhc.GetSelectedDiceHolder().getType() == DiceHolderType.OFFER) { // if it
											...
											exception = true;
											dhc.setEyes(7 - dhc.GetSelectedDiceHolder().getDie().getEyes(),
													dhc.GetSelectedDiceHolder().getDie());
											selectedToolcard = 10;
											psc.pay(cardpane.getCardNr(), tcc.getPrice(cardpane.getCardNr(), gameid)); // pay
										}
									}
								...
								if (dhc.GetSelectedDiceHolder() != null) {
									if (dhc.GetSelectedDiceHolder().getDie() != null) { // check if it has a die
										if (dhc.GetSelectedDiceHolder().getType() == DiceHolderType.OFFER) {
											...
											amountOfMoves = 0;
											waitTill = 2;
											// verplaats 2 dobbelstenen van dezelfde kleur die overeenkomen met een
											// steen op
											// het rondespoor
											psc.pay(cardpane.getCardNr(), tcc.getPrice(cardpane.getCardNr(), gameid));
										}
