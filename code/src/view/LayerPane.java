...
public class LayerPane extends BorderPane {// deze moet nog voor de gamepane worden aangemaakt(dus in de scene). deze
											// pane geeft de mogelijkheid om "pop-ups" te cre�ren(voor bijvoorbeeld
											// een pauze menu, patroonkaart keuze en een scorebord op einde van game
											// ~Rens
	...
	public LayerPane(LayerController controller, PatterncardController pcc, LoginController loginController,
			MyScene myscene, GameController gameController) {
	...
	private void viewOffer() {
		...
		chooserPane.getChildren().addAll(createPatternCard(String.valueOf(randomPat[0])),
				createPatternCard(String.valueOf(randomPat[1])), createPatternCard(String.valueOf(randomPat[2])),
				createPatternCard(String.valueOf(randomPat[3])));
	...
	private Pane createPatternCard(String rdInt) {
		...
		for (int i = 1; i < 5; i++) {
			for (int j = 1; j < 6; j++) {
				...
				patternCard.getChildren()
						.add(pcc.PatterncardCreate(j, i, Integer.parseInt(rdInt), (int) size, PatterncardType.CHOICE));
			}
		}
		...
		overlapPane.setOnMouseClicked(e -> {
			pcc.givePatternCardToPlayer(Integer.parseInt(rdInt), playerid); // Wanneer je klikt op de tilepane krijg je
																			// die id in de database bij player
