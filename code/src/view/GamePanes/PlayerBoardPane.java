...
public class PlayerBoardPane extends Pane {// misschien hernoemen naar bord pane? pattern card moet zich op het bord
											// bevinden toch? ~Rens
	...
	private void addPatternCard() {
		...
		for (int i = 1; i < 5; i++) {
			for (int j = 1; j < 6; j++) {
				double size = ((GamePane.windowMaxWidth / 3) / 5) - 1;
				patternCardView.getChildren()
						.add(pcc.PatterncardCreate(j, i, pcnumber, (int) size, PatterncardType.PLAYER));
			}
		}
	}
