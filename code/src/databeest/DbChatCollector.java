...
public class DbChatCollector {
	...
	public void sendChat(int playerid, String date, String message) {
		String query = "INSERT INTO `chatline` (`player_idplayer`, `time`, `message`) VALUES('" + playerid + "', "
				+ date + ", '" + message + "');";
	...
	public ArrayList<String> getChat(int amountOfPlayers, int playerid1, int playerid2, int playerid3, int playerid4) {
		...
		if (amountOfPlayers == 4) {
			query = "SELECT * FROM chatline WHERE player_idplayer = '" + playerid1 + "' OR player_idplayer = '"
					+ playerid2 + "' OR player_idplayer = '" + playerid3 + "' OR player_idplayer = '" + playerid4
					+ "' ORDER BY time;";
		} else if (amountOfPlayers == 3) {
			query = "SELECT * FROM chatline WHERE player_idplayer = '" + playerid1 + "' OR player_idplayer = '"
					+ playerid2 + "' OR player_idplayer = '" + playerid3 + "' ORDER BY time;";
		} else if (amountOfPlayers == 2) {
			query = "SELECT * FROM chatline WHERE player_idplayer = '" + playerid1 + "' OR player_idplayer = '"
					+ playerid2 + "' ORDER BY time;";
		}
		ArrayList<String> chat = dataBaseApplication.getChat(query);
		return chat;
	}

	public ArrayList<String> getChatDate(int amountOfPlayers, int playerid1, int playerid2, int playerid3,
			int playerid4) {
		String query = "";
		if (amountOfPlayers == 4) {
			query = "SELECT RIGHT(time, 8) FROM chatline WHERE player_idplayer = '" + playerid1
					+ "' OR player_idplayer = '" + playerid2 + "' OR player_idplayer = '" + playerid3
					+ "' OR player_idplayer = '" + playerid4 + "' ORDER BY time;";
		} else if (amountOfPlayers == 3) {
			query = "SELECT RIGHT(time, 8) FROM chatline WHERE player_idplayer = '" + playerid1
					+ "' OR player_idplayer = '" + playerid2 + "' OR player_idplayer = '" + playerid3
					+ "' ORDER BY time;";
			...
			query = "SELECT RIGHT(time, 8) FROM chatline WHERE player_idplayer = '" + playerid1
					+ "' OR player_idplayer = '" + playerid2 + "' ORDER BY time;";
		}
	...
	public ArrayList<Integer> getPlayers(int amountOfPlayers, int playerid1, int playerid2, int playerid3,
		...
		String query = "";
		if (amountOfPlayers == 4) {
			query = "SELECT player_idplayer FROM chatline WHERE player_idplayer = '" + playerid1
					+ "' OR player_idplayer = '" + playerid2 + "' OR player_idplayer = '" + playerid3
					+ "' OR player_idplayer = '" + playerid4 + "' ORDER BY time;";
		} else if (amountOfPlayers == 3) {
			query = "SELECT player_idplayer FROM chatline WHERE player_idplayer = '" + playerid1
